...
export const LandingPage = ({ onSuccess }) => {
  ...
  const googleLogin = useGoogleLogin({
    onSuccess: async (tokenResponse) => {
      setLoading(true);
      try {
        const { token, user, isNewUser } = await api.signIn({ access_token: tokenResponse.access_token });
    ...
    <div className="min-h-screen bg-neo-bg text-neo-text flex flex-col selection:bg-neo-neonGreen selection:text-black">
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            ...
            <button
              onClick={() => googleLogin()}
              disabled={loading}
              className="flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-bold bg-neo-surface border border-neo-border hover:border-neo-neonGreen/50 text-white hover:text-neo-neonGreen transition-all shadow-sm active:scale-95"
            >
              <GoogleIcon />
              <span>Sign In</span>
            </button>
      ...
      <main className="flex-1 max-w-6xl mx-auto px-4 sm:px-6 py-12 sm:py-20 space-y-20">
        <div className="text-center space-y-6 max-w-3xl mx-auto">
          ...
          {/* Clean Generic Google Login Button */}
          <div className="pt-4 flex flex-col items-center gap-3">
            <button
              onClick={() => googleLogin()}
              disabled={loading}
              className="flex items-center justify-center gap-3 px-7 py-3.5 bg-neo-surface hover:bg-neo-card border border-neo-border hover:border-neo-neonGreen/60 rounded-full font-bold text-sm text-white shadow-neo-glow-green transition-all hover:scale-[1.02] active:scale-95 group"
            >
              {loading ? (
                <>
                  <div className="w-4 h-4 border-2 border-neo-neonGreen border-t-transparent rounded-full animate-spin" />
                  <span className="text-neo-neonGreen">Signing in securely...</span>
                </>
              ) : (
                <>
                  <GoogleIcon />
                  <span className="group-hover:text-neo-neonGreen transition-colors">Continue with Google</span>
                </>
              )}
            </button>
        ...
        <div id="security" className="bg-gradient-to-br from-neo-surface to-neo-card border border-neo-border rounded-3xl p-8 sm:p-12 text-center space-y-6">
          ...
          <div className="flex justify-center">
            <button
              onClick={() => googleLogin()}
              disabled={loading}
              className="flex items-center justify-center gap-3 px-7 py-3.5 bg-neo-card hover:bg-neo-surface border border-neo-border hover:border-neo-neonGreen/60 rounded-full font-bold text-sm text-white shadow-neo-glow-green transition-all hover:scale-[1.02] active:scale-95"
            >
              <GoogleIcon />
